...
const Header = () => {
  ...
  return (
    <div className="fixed z-50 top-0 left-0 w-full h-40 flex justify-center text-xl font-semibold lg:left-1/2 lg:-translate-x-1/2">
      <div className="relative w-full overflow-visible h-23 px-5">
        <div className={`flex flex-row justify-center sm:justify-around items-center gap-10 sm:gap-0 lg:gap-10 h-full w-full mx-auto sm:w-full lg:w-95/100 backdrop-blur-sm ${showDropdown? '' : '[mask-image:linear-gradient(to_bottom,black_80%,transparent_100%)]'}`}>
      ...
      <div className={`absolute z-50 opacity-0 left-1/2 -translate-x-1/2  w-full sm:w-9/10 mx-auto sm:w-full lg:w-95/100 mx-auto backdrop-blur-sm rounded-3xl pb-2 flex flex-col justify-start items-center gap-5 md:hidden transition transition-all duration-300 ease-in-out
         ${showDropdown ? 'top-20 opacity-100 pointer-events-auto' : '-top-5 opacity-0 pointer-events-none'}`}>
